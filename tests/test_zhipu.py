...
class TestZhipu(BasicTestCase):
    ...
    def test_not_support_system_model(self):
        ...
        response = chat(model=model, messages=messages, stream=False, temperature=0.0)
    ...
    def test_lingxin_model(self):
        ...
        messages = [
            ...
            {"role": "assistant", "content": "你好，我是Emohaa，很高兴见到你。请问有什么我可以帮忙的吗？"},
            {"role": "user", "content": "最近我感觉压力很大，情绪总是很低落。"},
            {"role": "assistant", "content": "听起来你最近遇到了不少挑战。可以具体说说是什么让你感到压力大吗？"},
            {"role": "user", "content": "主要是工作上的压力，任务太多，总感觉做不完。"},
        ]
    ...
    def test_agent_chat(self):
        ...
        response = agent_chat(model=model, messages=question, tools=tools, stream=True, temperature=0.0, log_level="DEBUG")
    ...
    def test_tool_call(self):
        ...
        model = "glm-4-0520"
        tools = [CurrentContextToolDesc]
        response = chat(model=model, messages=question, tools=tools, stream=True, temperature=0.0)
        content = show_response(response)
        self.assertIsNotNone(response.tool_calls)
        self.assertGreater(len(response.tool_calls), 0)
        self.assertEqual(CurrentContextToolDesc.content_resp, content)
        self.assertEqual("current_context", response.tool_calls[0].name)
        for tool_call in response.tool_calls:
            resp = on_tool_call(tool_call)
            logger.info(f"tool call resp:{resp}")
        response = chat(model=model, messages=question, tools=tools, tool_calls=response.tool_calls, stream=True, temperature=0.0)
    ...
    def test_embd(self):
        ...
        show_embeds(embds)
        self.assertEqual(2, len(embds))
        self.assertEqual(512, len(embds[0]))
